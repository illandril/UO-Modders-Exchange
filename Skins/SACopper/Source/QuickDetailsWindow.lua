...
--[[
    ...
    CustomSettingsWindow.AddColorSetting( StringToWString("Quick Details Value Color"), QuickDetailsWindow.DetailLabelsValueColorSetting, { r=255, g=255, b=255 } )
...
function QuickDetailsWindow.UpdateLabelFonts()
    if QuickDetailsWindow.DetailLabelsCreated then
        ...
        local labelColor = CustomSettings.LoadColorValue( {settingName=QuickDetailsWindow.DetailLabelsLabelColorSetting, defaultValue={ r=255, g=255, b=255 } } )
...
function QuickDetailsWindow.UpdateLabels()
    ...
    if noneVisible then
        -- changed this to moving over if no extra labels so won't cover up the button
        nextAnchor.x = nextAnchor.x + 15
    end
    if QuickDetailsWindow.LastBuffDebuffAnchor ~= nextAnchor.relative then
        QuickDetailsWindow.LastBuffDebuffAnchor = nextAnchor.relative
        WindowClearAnchors( "BuffDebuff" )
        WindowAddAnchor( "BuffDebuff", "topleft", nextAnchor.relative, "topleft", nextAnchor.x, nextAnchor.y )
    end
end
