...
MapWindow.ComboBoxSet = 0
MapWindow.OldFacet = -1


MapWindow.LegendItemTextColors = { normal={r=192,g=162,b=117}, disabled={r=80,g=80,b=80} }
-----------------------------------------------------------------
...
function MapWindow.Initialize()
    ...
    WindowSetShowing("MapWindowShowComboBox", true)
    WindowSetShowing("MapWindowHideComboBox", false)
    ...
    WindowAddAnchor("Map", "topleft", "MapWindow", "topleft", 5, 54)

    MapWindow.PopulateMapLegend()
    
    local windowWidth, windowHeight = WindowGetDimensions("MapWindow")
    WindowSetDimensions( "Map", windowWidth - 10, windowHeight - 60)
...
function MapWindow.OnResizeBegin()
	local x, y = WindowGetDimensions( window )
		if ( MapWindow.ComboBoxSet ~= 0 ) then
			WindowUtils.BeginResize( "MapWindow", "topleft", 290, 385, false, MapWindow.OnResizeEnd )
		else
			WindowUtils.BeginResize( "MapWindow", "topleft", 290, 330, false, MapWindow.OnResizeEnd )
		end
end

function MapWindow.OnResizeEnd( window )
	local x, y = WindowGetDimensions( window )
		if ( MapWindow.ComboBoxSet ~= 0 ) then
			WindowSetDimensions("Map",x - 10, y - 114) 
		else
			WindowSetDimensions("Map",x - 10, y - 59) 
		end
end


function MapWindow.ShowComboBox()
	local width, height = WindowGetDimensions( "MapWindow" )
	MapWindow.ComboBoxSet = 1
	WindowSetShowing("MapWindowShowComboBox", false)
	WindowSetShowing("MapWindowHideComboBox", true)
	...
	WindowClearAnchors("Map")
	WindowAddAnchor("Map", "topleft", "MapWindow", "topleft", 5, 109)
	WindowSetDimensions("MapWindow", width, height + 55)
end

function MapWindow.HideComboBox()
	local width, height = WindowGetDimensions( "MapWindow" )
	MapWindow.ComboBoxSet = 0
	WindowSetShowing("MapWindowShowComboBox", true)
	WindowSetShowing("MapWindowHideComboBox", false)
