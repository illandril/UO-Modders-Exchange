...
function DebugWindow.Initialize()
    ...
    LogDisplaySetFilterColor("DebugWindowText", "UiLog", LuaLog.FUNCTION, 255, 175, 255 ) -- Green
    ...
    WindowSetAlpha("DebugWindow", 255)
...
function DebugWindow.ToggleLogging()
    ...
    local logEnabled = TextLogGetEnabled( "UiLog" )
    if( logEnabled ) then
...
function DebugWindow.OnResizeBegin()
    WindowUtils.BeginResize( "DebugWindow", "topleft", 300, 200, false, DebugWindow.OnResizeEnd)
end

function DebugWindow.OnResizeEnd()
	local windowWidth, windowHeight = WindowGetDimensions("DebugWindow")
