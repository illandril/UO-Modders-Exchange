...
CustomSettings.SettingsUpdateListeners = {}

function CustomSettings.Initialize()
    WindowRegisterEventHandler( "Root", SystemData.Events.USER_SETTINGS_UPDATED, "CustomSettings.UpdateSettings")
    CustomSettings.OriginalSettingsWindowApply = SettingsWindow.OnApplyButton
    SettingsWindow.OnApplyButton = CustomSettings.SettingsWindowApply
end

function CustomSettings.SettingsWindowApply()
    -- Why? Because the event doesn't seem to want to fire.
    CustomSettings.OriginalSettingsWindowApply()
    CustomSettings.UpdateSettings()
end

function CustomSettings.UpdateSettings()
    for i,fn in ipairs( CustomSettings.SettingsUpdateListeners ) do
        fn()
    end
end

function CustomSettings.RegisterUpdateSettingsListener( fn )
    table.insert( CustomSettings.SettingsUpdateListeners, fn )
end
