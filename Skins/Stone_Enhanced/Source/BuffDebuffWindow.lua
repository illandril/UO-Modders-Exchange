...
function BuffDebuff.ShouldCreateNewBuff()
	...
	if( BuffDebuff.retrieveBuffData( buffData ) ) then
		...
		-- Lucitus fix for show bad BuffDebuff icons: (the problem is that any new buff that shouldn't have a timer will be suppressed - if there really still is an issue, we should be checking for things that we know should have a timer, not checking for things we know shouldn't have a timer, and letting things through by default)
