...
function TargetWindow.UpdateStatus(targetId)	
	if( targetId == WindowData.CurrentTarget.TargetId ) then
		...
		if(TargetWindow.TargetType == TargetWindow.MobileType and WindowData.MobileStatus[WindowData.CurrentTarget.TargetId].IsDead == false ) then
			...
			if (data.Notoriety+1 < 8) then
					...
					LabelSetText("TargetWindowHits", StringToWString("Hits: " .. tostring(perc) .. "%"))
			else
